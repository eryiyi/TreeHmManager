    ...
    log.event = function () {
        ...
        log.tabimg = function () {
            ...
            if (json) {
                ...
                var txt = thisimg.attr('context') || "暂无数据";
                $(".xubox_intro").html('<div style="padding:20px;">' + txt + '<p>相册支持左右方向键，支持Esc关闭</p><p id="change"></p></div>');
...
layer.photosPage = function (options) {
    var log = {};
    log.run = function (index, htmlText) {
        layer.photos({
            html: htmlText,
    ...
    $(options.parent).find('img').each(function (index) {
        $(this).on('click', function () {
            var txt = $(this).attr("context") || "暂无描述";
            log.run(index, txt);
        });
    });
};