            ...
            <div class="box-content">
                ...
                <table class="table">
                    ...
                    <c:forEach items="${list}" var="e" varStatus="st">
                        <tr>
                            <td>${e.mm_notice_title}</td>
                            <td>${e.dateline}</td>
                            <td>
                                <a class="btn btn-default btn-sm" href="javascript:void (0)"
                                   onclick="editRole('${e.mm_notice_id}')" role="button">详情</a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script type="text/javascript">
    function searchIndex(e) {
        ...
        if (_index <= ${page.pageCount} && _index >= 1) {
            window.location.href = "#module=/noticeController/list&page=" + _index + "&size=" + size + "&_t=" + new Date().getTime();
        } else {
    ...
    function nextPage(_page) {
        ...
        if ((page <= ${page.pageCount} && page >= 1)) {
            window.location.href = "#module=/noticeController/list&page=" + page + "&size=" + size + "&_t=" + new Date().getTime();
        } else {
            alert("请输入1-${page.pageCount}的页码数");
        }
    }
    ...
    function editRole(_id) {
        $.ajax({
            type: "GET",
            data: {"id": _id},
            url: "/noticeController/toDetail.do",
